...
from pyspark.sql.functions import (
    col, from_json, from_unixtime, to_timestamp, expr, to_json, struct, lit, when, pow, hour
)
...
os.environ["SPARK_HOME"] = "/workspaces/TP_kakfa2/spark-3.2.3-bin-hadoop2.7"
...
# Calcul de "heat_index"
processed_df = processed_df.withColumn(
    "heat_index",
    col("temperature") + (
        0.5555 * (
            (6.11 * pow(10, (7.5 * col("temperature")) / (237.7 + col("temperature")))) * (col("humidity") / 100) - 10
        )
    )
)

# Calcul de "severity_index"
processed_df = processed_df.withColumn(
    "severity_index",
    (col("speed") * 0.5) + ((lit(1015) - col("pressure")) * 0.3) + (col("humidity") * 0.2)
)

# Ajout de la colonne "time_of_day"
processed_df = processed_df.withColumn(
    "time_of_day",
